...
void introsort(int* arr, int N, int left, int right, int depth){//quick_sort + heap_sort
    
    if (depth == 0) {heap_sort(arr, N); return;}
    else{
        if (left >= right) return;
        int pivot = partition(arr, left, right);

        introsort(arr, N, left, pivot-1, depth-1);
        introsort(arr, N, pivot+1, right, depth-1);
    }
}
...
//결과 : 시간초과, 그냥 quick_sort만 쓴 것보다 느린듯